...
void Enemy::Update(const float& dt)
{
	...
	if (Input::GetKeyTrigger('G')) { Parent->Destroy(); }
}
...
void Enemy::Draw()
{
	...
	// ワールド行列を計算
	XMMATRIX scl = DirectX::XMMatrixScaling(Parent->GetComponent<Transform>()->scale.x, Parent->GetComponent<Transform>()->scale.y, Parent->GetComponent<Transform>()->scale.z);
	XMMATRIX rot = Matrix::CreateFromQuaternion(Parent->GetComponent<Transform>()->rotation);
	XMMATRIX trans = DirectX::XMMatrixTranslation(Parent->GetComponent<Transform>()->position.x, Parent->GetComponent<Transform>()->position.y, Parent->GetComponent<Transform>()->position.z);
...
void Enemy::CompInfo()
{
	...
	SeparatorText("EnemyComponent");

	if (TreeNode("EnemyInfo")) {
